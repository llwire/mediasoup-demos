...
function startGStreamerRtmpStream() {
  ...
  let gstreamerArgs = [
    ...
    `rtmpsink sync=false location="${global.gstreamer.rtmpTarget}${testFlag} live=1" demux. !`,
    `queue ! progressreport update-freq=10`,
  ].join(' ').trim();
  ...
  let gstreamerEnv = {
    GST_DEBUG: '*:2,sdpdemux:6,flvmux:4,rtmpsink:6', // log level 4 = INFO
  }
